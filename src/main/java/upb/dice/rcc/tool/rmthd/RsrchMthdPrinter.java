...
public abstract class RsrchMthdPrinter {
	...
	public static void printResults(Map<String, Set<String>> resIdMap) throws IOException {
		...
		try {
			...
			ObjectNode inpObj = (ObjectNode) RccFrmtRsrchMthdMdlGnrtr.OBJ_READER.readTree(fin);
			...
			while (methodItr.hasNext()) {
				...
				Set<String> fileNames = resIdMap.get(methodEle.get(RccFrmtRsrchMthdMdlGnrtr.ID_FLD).asText());
				if (fileNames != null) {
					String methodLabel = methodEle.get(LABEL_FLD).get(RccFrmtRsrchMthdMdlGnrtr.VALUE_FLD).asText();
